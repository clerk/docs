...
As a child of `<SignedOut/>`, we will include our [`<SignInWithMetamaskButton/>`](../components/less-than-signinwithmetamaskbutton-greater-than.md) so the user can trigger the Metamask authentication flow.
...
Update the **.env.local** in your application root and add your `CLERK_API_KEY`.  You can retrieve the value in your Dashboard **API Keys** section.
