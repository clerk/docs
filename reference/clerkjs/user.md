...
Finally, `User` objects hold profile data like their name, a profile image and a set of metadata that can be used internally to store arbitrary information. The metadata are split into public and private. Both types are set from the [Backend API](signin/backend-api-reference/), but public metadata can be accessed from the [Frontend API](../frontend-api-reference/) and [Backend API](signin/backend-api-reference/).
...
| **publicMetadata**        | <p><em>{[string]: any} | null</em></p><p>Metadata that can be read from the <a href="../frontend-api-reference/">Frontend API</a> and <a href="signin/backend-api-reference/">Backend API</a> and can be set only from the Backend API .</p>                                                                                                                                                                                                                                      |
| **privateMetadata**       | <p><em>{[string]: any} | null</em></p><p>Metadata that can be read and set only from the <a href="signin/backend-api-reference/">Backend API</a>.</p>                                                                                                                                                                                                                                                                                                                             |
...
| **options?** | <p><em></em><a href="user.md#usergettokenoptions"><em>UserGetTokenOptions</em></a><em></em></p><p>Optionally pass the leeway for expiring the cache. Default leeway is 10 seconds and cannot exceed the token TTL, which is 1 minute.</p> |
{% endtab %}
...
`facebook | github | google | hubspot | tiktok | gitlab | discord | twitter`
